...
public class Main {
    public static void main(String[] args) {
        try (Connection conn = DBConnection.getConnection();
            ...
            int playerId;
            String selectQuery = "SELECT playerID, wallet, current_day, current_season, current_year FROM player WHERE playername = ?";
            ...
            if (rs.next()) {
                ...
                String insertQuery = "INSERT INTO player (playername, wallet, current_day, current_season, current_year) VALUES (?, ?, ?, ?, ?)";
    ...
    try {
        PreparedStatement stmt = conn.prepareStatement(
            "SELECT current_day, current_season, current_year FROM player WHERE playerID = ?"
        );
        ...
        if (rs.next()) {
            ...
            PreparedStatement updateStmt = conn.prepareStatement(
                "UPDATE player SET current_day = ?, current_season = ?, current_year = ? WHERE playerID = ?"
            );
   ...
   public static void showFarm(Connection conn, int playerId, Scanner scanner) {
    try {
        ...
        String cropQuery = "SELECT c.cropID, i.itemname, c.growth_time, c.readytoharvest FROM crop c JOIN items i ON c.itemID = i.itemID WHERE c.playerID = ?";
        ...
        switch (farmChoice) {
            case "1":
                ...
                String seedQuery = "SELECT i.itemID, i.itemname, inv.quantity, i.specialvalue FROM inventory inv JOIN items i ON inv.itemID = i.itemID WHERE inv.playerID = ? AND i.itemtype = 'crop'";
            ...
            case "3":
                ...
                String animalInvQuery = "SELECT i.itemID, i.itemname, inv.quantity, i.specialvalue FROM inventory inv JOIN items i ON inv.itemID = i.itemID WHERE inv.playerID = ? AND i.itemtype = 'animal'";
    ...
    try {
        ...
        String checkQuery = "SELECT inv.quantity, i.itemname, i.specialvalue FROM inventory inv JOIN items i ON inv.itemID = i.itemID WHERE inv.playerID = ? AND inv.itemID = ? AND i.itemtype = 'crop'";
...
public static void placeanimal(Connection conn, int playerId, int itemID) {
    try {
        ...
        String checkQuery = "SELECT inv.quantity, i.itemname, i.specialvalue FROM inventory inv JOIN items i ON inv.itemID = i.itemID WHERE inv.playerID = ? AND inv.itemID = ? AND i.itemtype = 'animal'";
        PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
        checkStmt.setInt(1, playerId);
        checkStmt.setInt(2, itemID);
        ResultSet rs = checkStmt.executeQuery();
    ...
    try {
        ...
        String checkQuery = "SELECT itemID, cropname, readytoharvest FROM crop WHERE cropID = ? AND playerID = ?";
        PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
        ...
        if (rs.next() && rs.getBoolean("readytoharvest")) {
            ...
            String deleteCrop = "DELETE FROM crop WHERE cropID = ?";
            ...
            System.out.println("That crop is not ready to harvest.");
        }
    } catch (SQLException e) {
...
public static void tendAnimal(Connection conn, int playerId, int animalID) {
    ...
    try {
        ...
        String updateGrowth = "UPDATE crop SET growth_time = growth_time - 1 WHERE playerID = ? AND readytoharvest = FALSE AND growth_time > 0";
        PreparedStatement updateStmt = conn.prepareStatement(updateGrowth);
        updateStmt.setInt(1, playerId);
        updateStmt.executeUpdate();
        // Set readytoharvest = TRUE where growth_time <= 0
        String readyQuery = "UPDATE crop SET readytoharvest = TRUE WHERE playerID = ? AND growth_time <= 0";
...
public static void updateAnimalGrowth(Connection conn, int playerId) {
        ...
        switch (shopChoice) {
            case "1":
                ...
                try {
                    ...
                    // Check player wallet
                    String walletQuery = "SELECT wallet FROM player WHERE playerID = ?";
                    ...
                    // Deduct money
                    String updateWallet = "UPDATE player SET wallet = wallet - ? WHERE playerID = ?";
                    ...
                    String upsertInv = "INSERT INTO inventory (playerID, itemID, quantity) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + ?";
                    PreparedStatement upsertStmt = conn.prepareStatement(upsertInv);
                    upsertStmt.setInt(1, playerId);
    ...
    try {
        ...
        String updateInv = "UPDATE inventory SET quantity = quantity - ? WHERE playerID = ? AND itemID = ?";
        PreparedStatement updateStmt = conn.prepareStatement(updateInv);
